...
- **Tor SOCKS proxy is written reactively.** The Config action records the user's intent (`outgoing.using_tor_proxy`); `init/watchTorProxy.ts` resolves tor's container IP (`sdk.getContainerIp`) and writes the actual `outgoing.proxies` URL into `settings.yml`, reacting to both the toggle and tor's IP changing. Tor's SOCKS is not a StartOS binding, so there is no host id to read.
